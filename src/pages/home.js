...
function buildCoverageCarousel() {
  ...
  const tickerWrap = document.createElement('div');
  tickerWrap.className = 'coverage-ticker-wrap';
  ...
  const afterTicker = document.createElement('div');
  afterTicker.className = 'container';
  afterTicker.innerHTML = `
    <div class="coverage-cta-wrap">
      <a href="/recent-work" class="coverage-cta">See More Recent Coverage →</a>
    </div>
  `;
  section.appendChild(afterTicker);
  ...
  requestAnimationFrame(() => {
    createTicker(
      ...
      { direction: 'left', speed: 35, gap: '24px' }
    );
  });
